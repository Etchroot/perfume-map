...
단순한 키워드 검색이 아닌, **3D 홀로그램 파티클 시각화** 위에서 AI 매칭 과정이 애니메이션으로 펼쳐지는 것이 핵심입니다. 약 400개의 향료 대분류(fragrance)가 우주의 별처럼 공간에 떠 있고, 검색 결과가 도출될수록 점들이 수렴·폭발하며 최종 향료를 가리킵니다.
...
### Step 0 — 초기 화면
- 약 400개의 빛나는 파티클이 3D 공간에 피보나치 구면 분포로 배치
...
| 컬럼 | 설명 | 비고 |
|------|------|------|
| `fragrance` | 향 대분류 | 약 400개 고유값 |
| `modifier` | 향 소분류 묘사 | AI 2차 매칭 대상 |
| `ingredient` | 실제 향료 원료명 | 최종 결과값 |

Firestore 컬렉션 구조:
- **`categories`**: fragrance 고유값 + 임베딩 벡터 + 3D 위치 좌표 (약 400개)
...
└── components/
    ...
    ├── ParticleField.jsx    # 400개 fragrance 파티클 (animateFilter, reset)
